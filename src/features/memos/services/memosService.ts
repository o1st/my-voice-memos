...
export class MemosService {
  ...
  async createMemo(memoData: { title: string; description: string }): Promise<ServiceResult<Memo>> {
  ...
  async updateMemo(id: string, updateData: { title?: string; description?: string }): Promise<ServiceResult<Memo>> {
    if (!id || typeof id !== 'string') {
      return {
        success: false,
        error: 'Invalid memo ID'
...
export const memosService = new MemosService(new LocalStorageRepository<Memo>({
